...
/**
 * To test this internal client, we will have to set the JACK_INTERNAL_DIR environment variable before
 * starting the jack server:
 *
 *  $ export JACK_INTERNAL_DIR="/home/harald/workspace/CLionExperiments/jack_examples/cmake-build-debug"
 *  $ jackd -dalsa -r44100 -p2048 -n3 -Xseq -D -Chw:USB -Phw:USB -i2 -o2
 *
 *  in an other terminal we can start the inprocess-client using `jack_load` like so:
 *
 *  $ jack_load libinternal
 *
 *  to stop the server we'll go into the first terminal and type Ctrl-C
 *
 */
#include <stdlib.h>
#include <stdio.h>
...
jack_initialize (jack_client_t *client, const char *load_init)
{
    ...
    pp->input_port = jack_port_register (client, "libinput",
                                         JACK_DEFAULT_AUDIO_TYPE,
                                         JackPortIsInput, 0);
    pp->output_port = jack_port_register (client, "liboutput",
                                          JACK_DEFAULT_AUDIO_TYPE,
                                          JackPortIsOutput, 0);
